...
export const getAdminReports = async (req, res) => {
    try {
        ...
        const coursePerformanceList = await Promise.all(courses.map(async (c, index) => {
        ...
        const mappedUsers = await Promise.all(recentUsers.map(async (u) => {
        ...
        const topLecturers = await Promise.all(lecturers.map(async (l) => {
