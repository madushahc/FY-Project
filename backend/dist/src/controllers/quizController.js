...
export const startQuizAttempt = async (req, res) => {
    try {
        ...
        const previousAttempts = await QuizAttempt.find({ quiz: quizId, student: studentId }).sort({ createdAt: -1 });
...
export const autoSaveAnswer = async (req, res) => {
    try {
        ...
        const existingIdx = attempt.answersSaved.findIndex((a) => a.questionId.toString() === questionId.toString());
        const now = new Date();
...
export const getMyQuizAttempts = async (req, res) => {
    try {
        const quizId = req.params.id;
        const studentId = req.user?._id;
        const attempts = await QuizAttempt.find({ quiz: quizId, student: studentId }).sort({ createdAt: -1 });
        res.json(attempts);
    }
    catch (error) {
        res.status(500).json({ message: "Server Error", error: error?.message });
...
export const submitQuizAttempt = async (req, res) => {
    try {
        ...
        quiz.questions.forEach((q) => {
            ...
            const submittedAns = submittedAnswersList.find((a) => a.questionId && a.questionId.toString() === q._id.toString()) || attempt.answersSaved?.find((a) => a.questionId && a.questionId.toString() === q._id.toString());
            let isCorrect = false;
        ...
        if (attempt.answersSaved && attempt.answersSaved.length > 0) {
            ...
            attempt.answersSaved.forEach((savedItem) => {
        ...
        try {
            const populatedQuiz = await Quiz.findById(quizId).populate("course");
...
export const getQuizStats = async (req, res) => {
    try {
        const { id } = req.params;
        const attempts = await QuizAttempt.find({ quiz: id, status: { $in: ['Completed', 'Submitted', 'Timed Out'] } });
...
// @access  Student
export const getAssignedFinalQuiz = async (req, res) => {
    try {
        ...
        const studentProgressDocs = await StudentProgress.find({ course: courseId, student: studentId });
        const completedLessonIds = new Set();
        if (enrollment?.completedLessons) {
            enrollment.completedLessons.forEach((id) => completedLessonIds.add(String(id)));
        }
        ...
        studentProgressDocs.forEach((sp) => {
            if (sp.answeredQuestions && Array.isArray(sp.answeredQuestions)) {
                sp.answeredQuestions.forEach((ans) => {
        ...
        let assignedQuiz = allCourseQuizzes.find(q => (q.isFinalQuiz || q.title.toLowerCase().includes('final')) && q.difficultyLevel === assignedDifficulty);
