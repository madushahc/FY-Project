...
import Course, { ILesson } from "../models/Course.js";
...
const findLessonInCourse = (course: any, lessonId: string): { lesson: ILesson | null; moduleId: string | null } => {
  if (!course || !course.modules) return { lesson: null, moduleId: null };
...
const findOrCreateStudentProgress = async (student: string, course: string, lessonId: string) => {
...
const checkAndMarkLessonCompletion = async (studentId: string, courseId: string, lessonId: string, lesson: ILesson) => {
  ...
  if (hasAnyRequirement && allQuestionsPassed && isVideoSatisfied && isReadingSatisfied) {
    ...
    if (enrollment) {
      ...
      if (!alreadyCompleted) {
        ...
        const completedCount = enrollment.completedLessons.filter((id: any) => validLessonIds.has(String(id))).length;
...
export const recordQuestionAnswer = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
export const recordWatchProgress = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const courseId = String(req.params.courseId || "");
    const lessonId = String(req.params.lessonId || "");
...
export const getInteractiveAnalytics = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const allLessonIds = courses
      .flatMap((c) =>
        (c.modules || []).flatMap((m: any) =>
          (m.lessons || []).map((l: any) => l._id?.toString() || l.title)
        )
      )
      .filter(Boolean);
    ...
    enrollments.forEach((e) => {
      if (e.student && (e.student as any)._id) {
        const sObj = e.student as any;
        ...
        if (!studentPerfMap[sId]) {
          studentPerfMap[sId] = {
            ...
            courseName: (e.course as any)?.title || "Course",
      ...
      if (sObj && sObj._id) {
        ...
        if (!studentPerfMap[sId]) {
          studentPerfMap[sId] = {
            ...
            email: sObj.email || "",
            profilePhoto: sObj.profilePhoto,
            courseName: (p.course as any)?.title || "Course",
            participantCode: sObj.participantCode || "",
            isResearchParticipant: sObj.isResearchParticipant !== false,
            isTestUser: sObj.isTestUser === true,
            totalQuestionsAttempted: 0,
        ...
        if (dateStr) sp.activeDaysSet.add(dateStr);
    ...
    allActivities.forEach((act) => {
      ...
      if (sId && studentPerfMap[sId]) {
        const sp = studentPerfMap[sId];
        if (act.activityType === 'login') sp.loginCount += 1;
        if (act.timestamp) {
          const actDate = new Date(act.timestamp);
          const actDateStr = actDate.toISOString().split("T")[0];
          if (actDateStr) sp.activeDaysSet.add(actDateStr);
          if (!sp.firstActiveDate || actDate < new Date(sp.firstActiveDate)) sp.firstActiveDate = actDate;
          if (!sp.lastActiveDate || actDate > new Date(sp.lastActiveDate)) sp.lastActiveDate = actDate;
        }
      }
    });
    ...
    allQuizAttempts.forEach((qa) => {
      ...
      if (sId && studentPerfMap[sId]) {
        ...
        if (qa.passed) sp.quizzesPassedCount += 1;
        else sp.quizzesFailedCount += 1;
        sp.quizScoreSum += qa.score || 0;
        if ((qa.quiz as any)?.isFinalQuiz) {
    ...
    const rawStudentList = Object.values(studentPerfMap)
      .map((sp, idx) => {
        ...
        const systemInteractionScoreS = Math.min(
          100,
          Math.round((((activeDaysCount / RESEARCH_CONFIG.RESEARCH_INTERVENTION_DAYS) * 50) + ((totalActivityEvents / RESEARCH_CONFIG.MEANINGFUL_INTERACTION_REFERENCE) * 50)) * 100) / 100
        );

        // Overall Composite Engagement Score E = 0.40V + 0.30Q + 0.20L + 0.10S
        const overallEngagementScoreE = Math.min(
          100,
          Math.round(((RESEARCH_CONFIG.WEIGHTS.V * videoScoreV) + (RESEARCH_CONFIG.WEIGHTS.Q * quizScoreQ) + (RESEARCH_CONFIG.WEIGHTS.L * lessonScoreL) + (RESEARCH_CONFIG.WEIGHTS.S * systemInteractionScoreS)) * 100) / 100
        );
        ...
        const riskReasonsList: string[] = [];
        if (overallEngagementScoreE < RESEARCH_CONFIG.AT_RISK.E_THRESHOLD) riskReasonsList.push(`Low Composite Engagement Score (<${RESEARCH_CONFIG.AT_RISK.E_THRESHOLD})`);
        if (videoScoreV < RESEARCH_CONFIG.AT_RISK.V_THRESHOLD) riskReasonsList.push(`Low Video Progress (<${RESEARCH_CONFIG.AT_RISK.V_THRESHOLD})`);
        if (quizScoreQ < RESEARCH_CONFIG.AT_RISK.Q_THRESHOLD) riskReasonsList.push(`Low Assessment Participation (<${RESEARCH_CONFIG.AT_RISK.Q_THRESHOLD})`);
    ...
    sortedByXP.forEach((item, idx) => {
      ...
      (item as any).leaderboardPosition = compRank;
    });
    ...
    const courseSummariesMap: Record<string, any> = {};
    courses.forEach((c) => {
      ...
      courseSummariesMap[cId] = {
        ...
        courseCode: c.code,
        courseTitle: c.title,
        instructorName: (c.instructor as any)?.name || "Lecturer",
    ...
    enrollments.forEach((e) => {
      const cId = e.course?._id?.toString() || (e.course as any)?.toString();
    ...
    studentRankings.forEach((s) => {
      const matchingCourse = Object.values(courseSummariesMap).find(
        (c) => c.courseTitle === s.courseName
      );
      if (matchingCourse) {
        matchingCourse.engagementScoreSum += s.engagementScore;
        matchingCourse.totalQuestionsAnswered += s.totalQuestionsAttempted;
        if (s.isAtRisk) matchingCourse.atRiskCount += 1;
      }
    });
    ...
    const avgEngagementScore =
      studentRankings.length > 0
        ? Math.round(
            studentRankings.reduce((sum, s) => sum + s.engagementScore, 0) /
    ...
    const avgLessonCompletionRate =
      studentRankings.length > 0
        ? Math.round(
            studentRankings.reduce((sum, s) => sum + s.completionRate, 0) /
              studentRankings.length
          )
        : 0;

    const questionParticipationRate =
      totalQuestionsConfigured > 0 && totalEnrolled > 0
        ? Math.min(
            100,
            Math.round(
    ...
    allProgress.forEach((p) => {
      ...
      if (pDate) {
        const dStr = (new Date(pDate).toISOString().split("T")[0] || "") as string;
    ...
    const engagementTrends = Array.from(trendsMap.values()).map((t) => {
      const dayAccuracy =
        t.questionsAnswered > 0
          ...
          : 0;
      const dayCompletion =
        t.progressCount > 0 ? Math.round(t.watchSum / t.progressCount) : 0;
      const score =
        t.progressCount > 0
    ...
    const selectedCourseObj = (courseId && courseId !== "all")
      ...
      : courses.find((c) => (c.modules || []).some((m: any) => m.lessons?.length > 0)) || courses[0];
    let selectedModuleObj: any = null;
    let selectedLessonObj: any = null;

    if (selectedCourseObj) {
      const courseModules = selectedCourseObj.modules || [];
      if (moduleId && typeof moduleId === "string" && moduleId !== "all") {
        selectedModuleObj = courseModules.find(
          (m: any) => (m._id?.toString() || m.title) === moduleId
        );
      }
      ...
      if (selectedModuleObj) {
        ...
        if (lessonId && typeof lessonId === "string" && lessonId !== "all") {
          selectedLessonObj = moduleLessons.find(
            (l: any) => (l._id?.toString() || l.title) === lessonId
          );
        }
    ...
    const totalMaxWatchedTimeSum = allProgress.reduce(
      (sum, p) => sum + (p.maxWatchedTime || 0),
      0
    );
    const avgWatchTimeSecs =
      allProgress.length > 0
        ? Math.round(totalMaxWatchedTimeSum / allProgress.length)
        : 0;
    const avgWatchTimeMins = Math.round(avgWatchTimeSecs / 60);

    const studentsWithXpOrScans = studentRankings.filter(
      (s) => s.totalPoints > 0
    ).length;
    const gamificationActivityRate =
      totalEnrolled > 0
    ...
    const moduleLessons = selectedModuleObj?.lessons || [];
    const videoComparison = moduleLessons.map((l: any) => {
      const lId = l._id?.toString() || l.title;
      const lProgress = allProgress.filter((p) => p.lessonId === lId);
      const complCount = lProgress.filter(
        (p) => p.completed || p.videoWatched
      ).length;
      ...
      const engPct =
        lProgress.length > 0
          ? Math.round(
              lProgress.reduce((sum, p) => sum + (p.watchPercent || 0), 0) /
                lProgress.length
            )
          : 0;
      return {
    ...
    const lowestEngVideo = videoComparison.reduce(
      (min: any, curr: any) =>
        parseInt(curr.engagementScore) <
        parseInt(min?.engagementScore || "100%")
          ? curr
          : min,
      videoComparison[0] || null
    );

    const moduleCompletionAvg =
      videoComparison.length > 0
        ? Math.round(
            videoComparison.reduce(
              (sum: number, v: any) => sum + parseInt(v.completionRate),
              0
            ) / videoComparison.length
          )
        : 0;

    const moduleEngagementAvg =
      videoComparison.length > 0
        ? Math.round(
            videoComparison.reduce(
              (sum: number, v: any) => sum + parseInt(v.engagementScore),
              0
            ) / videoComparison.length
          )
        : 0;
    ...
    const targetLessonId =
      selectedLessonObj?._id?.toString() || selectedLessonObj?.title || "";
    ...
    const videoCompletedCount = videoProgressList.filter(
      (p) => p.completed || p.videoWatched
    ).length;
    const videoCompletionPct =
      videoStartedCount > 0
        ? Math.round((videoCompletedCount / videoStartedCount) * 100)
        : 0;
    const videoAvgDurationMins =
      videoStartedCount > 0
        ? Math.round(
            videoProgressList.reduce(
              (sum, p) => sum + (p.maxWatchedTime || 0),
              0
            ) /
              videoStartedCount /
      ...
      (c.modules || []).forEach((m: any) => {
        (m.lessons || []).forEach((l: any) => {
          if (
            ...
            (l.questionMarkers || []).forEach((q: any) => {
              ...
              const qCorrectPct =
                qAsked > 0 ? Math.round((qCorrect / qAsked) * 100) : 0;
              const qWrongPct = Math.max(0, 100 - qCorrectPct);
              const qAvgTime =
                qAnswers.length > 0
                  ? Math.round(
                      qAnswers.reduce((sum, a) => sum + (a.timeTaken || 0), 0) /
                        qAnswers.length
                    )
                  : 0;

              let difficulty = "Low";
              if (qCorrectPct < 50) difficulty = "High";
              else if (qCorrectPct <= 75) difficulty = "Medium";
    ...
    const highXpAvgEng =
      highXpStudents.length > 0
        ? Math.round(
            highXpStudents.reduce((sum, s) => sum + s.engagementScore, 0) /
    ...
    const lowXpAvgEng =
      lowXpStudents.length > 0
        ? Math.round(
            lowXpStudents.reduce((sum, s) => sum + s.engagementScore, 0) /
    ...
    const mediumEngaged = studentRankings.filter(
      (s) => s.engagementScore >= 50 && s.engagementScore < 80
    );
    ...
    const researchInsights: string[] = [];
    if (videoLevel.videoTitle && videoLevel.highestDropoff !== "N/A") {
      researchInsights.push(
        `Students lost engagement after the ${videoLevel.highestDropoff} mark in "${videoLevel.videoTitle}".`
      );
    }
    const hardQuestion = dynamicQuestionsList.find((q) => q.difficulty === "High");
    if (hardQuestion) {
      researchInsights.push(
        `Question "${hardQuestion.questionText}" has a low correct response rate (${hardQuestion.correctPct}) and may require content improvement.`
      );
    }
    if (highXpAvgEng > 0) {
      researchInsights.push(
        `Students with higher XP points (${highXpAvgEng}%) show higher lesson completion than low XP students (${lowXpAvgEng}%).`
      );
    }
    researchInsights.push(
      `Longer videos (> 15 minutes) exhibit lower completion rates compared to shorter clips (< 8 minutes).`
    );
    ...
    res.json({
      summary: {
        ...
        totalQuestionsAttempted,
        totalQuestionsCorrect,
        totalQuestionsIncorrect: Math.max(
          0,
        ...
        totalSkippedInteractions: Math.max(
          0,
          totalQuestionsConfigured * totalEnrolled - totalQuestionsAttempted
        ),
        avgLessonRating: 4.8,
...
export const exportResearchData = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const studentIds = Array.from(new Set(
      enrollments
        .map(e => (e.student as any)?._id?.toString())
        .filter(Boolean)
    ));
    ...
    const studentMetrics = students.map((s, idx) => {
      ...
      const uniqueQuizzesAttempted = new Set(sQuizzes.map(q => (q.quiz as any)?._id?.toString() || (q.quiz as any)?.toString())).size;
      ...
      const finalQuizObj = sQuizzes.find(q => (q.quiz as any)?.isFinalQuiz);
...
export const exportRawEventsData = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const courseIdParam = req.query.courseId ? String(req.query.courseId) : undefined;
    ...
    const query: any = {};
    if (courseIdParam) query.course = courseIdParam;
    ...
    const filtered = rawActivities.filter(a => {
      const sObj = a.student as any;
      const cObj = a.course as any;
